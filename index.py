...
options = Options()
PATH = "C:/Users/ekn/Downloads/edgedriver_arm64/msedgedriver.exe"
...
for i in json_list:
    ...
    text_clip = TextClip(text, font="Arial", fontsize=72, color='white', align='center', method='caption', size=(1920 - 2 * padding, None)).set_duration(duration-1)
...
# Position the text clip horizontally centered and at the bottom
    ...
    for image_path in image_paths:
        image_clip = ImageClip(image_path, duration=(duration-1)/3).resize((1920, 1080))  # Match text duration and size
    ...
    *[(clip.set_position(("center", "center")).set_start(idx * 5)) for idx, clip in enumerate(image_clips)],
    ...
    audio_clip = AudioFileClip(f"C:/Users/ekn/Desktop/github-projects/video creator/audio/{i['google_phrase']}.wav")
...
# Write the final clip to a video file
    final_clip.write_videofile(f"C:/Users/ekn/Desktop/github-projects/video creator/video/{i['google_phrase']}.mp4", fps=24)
    video_paths.append(f"C:/Users/ekn/Desktop/github-projects/video creator/video/{i['google_phrase']}.mp4")
...
for path, title_text in zip(video_paths, title):
    video_clip = VideoFileClip(path)
    title_clip = TextClip(title_text, fontsize=70, color='white', size=video_clip.size)
    title_duration = 3  # Duration of each title page (in seconds)
    title_clip = title_clip.set_duration(title_duration)
    video_clips.extend([title_clip, video_clip])
video_clips.pop(0)
